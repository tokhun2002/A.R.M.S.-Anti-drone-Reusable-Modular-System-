...
이거 하나로 다음을 전부 띄운다 (PX4 SITL 제외, 그건 무거워서 따로):
  ...
  - fusion_detector.py      (YOLO+CV 융합검출, 모드 yolo/cv/both)
  - balloon_referee.py      (풍선 비행 + 명중 연출)
  - arms_panel.py           (컨트롤 패널 GUI — 시작하자마자 뜸)

전제: 환경변수 ARMS_SW 가 SW 폴더(=tools, simulation 들어있는 곳)를 가리켜야 함.
      run_arms.sh 가 자동으로 export 해줌.
"""
...
def generate_launch_description():
    ...
    if tools and tools.is_dir():
        actions += [
            ...
            ExecuteProcess(cmd=["python3", str(tools / "arms_panel.py")], output="screen"),
        ]
    else:
        actions.append(LogInfo(msg=(
            "[arms_sitl_flying] ARMS_SW 환경변수가 안 잡혀서 검출기/풍선/패널을 못 띄웠음. "
