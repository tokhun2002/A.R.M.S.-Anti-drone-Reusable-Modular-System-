# arms_controller_input

Jetson Orin Nano Super에서 조종기 입력을 읽어 ROS2 `/joy` 토픽으로 publish하는 패키지이다.

- ADS1115 A0~A3 → `sensor_msgs/msg/Joy.axes[0..3]`
- GPIO 스위치 3개 → `sensor_msgs/msg/Joy.buttons[0..2]`
- 출력 토픽: `/joy`
...
```text
arms_controller_input/config/controller_input_params.yaml
```
...
Jetson 40핀 기준으로는 29번, 31번, 33번 핀을 사용한다.
...
colcon build --symlink-install --packages-select arms_controller_input
...
```bash
ros2 launch arms_controller_input controller_input.launch.py
```
