...
### `crsf_dump.py` 는 `Hz` 칸을 봐야 한다

프레임이 실제로 들어오는 중인지는 **`Hz` 칸**으로만 판단한다.
수신이 끊기면 `0.0 Hz  ---- 수신 없음 ----` 이 찍힌다. 줄이 계속 올라오는 것 자체는
수신 여부와 무관하다 (주기적으로 상태를 보고할 뿐).

또한 배선을 끊어도 **1~2초 정도는 정상적으로 프레임이 더 들어온다.**
커널 tty 버퍼(보통 4KB)에 남아 있던 데이터가 빠지는 것으로,
100Hz × 26바이트 = 2.6KB/s 기준 약 1.5초 분량이다. 그 이상 계속되면 배선이 덜 끊긴 것.

### 오실로스코프로 UART TX 를 볼 때

420000 baud 에서 **1비트 = 약 2.4µs** 다. 프레임 하나(26바이트 ≈ 260비트)는 약 620µs 이고,
100Hz 송신이면 10ms 마다 짧게 터지고 나머지 94% 는 idle 이다.
그래서 timebase 를 ms/div 에 두면 디지털 파형이 아니라 **뭉개진 띠**처럼 보인다.

| 항목       | 권장                                          |
| ---------- | --------------------------------------------- |
| timebase   | 비트 관찰 `2~5 µs/div` / 프레임 전체 `200 µs/div` |
| 트리거     | **falling edge, Normal** (Auto 면 free-run 해서 쓰레기처럼 보임) |
| idle 레벨  | 깨끗한 **3.3V High**. 시작 비트에서 Low 로 떨어짐 |
| 프로브 GND | Jetson GND 에 확실히 물릴 것 (뜨면 링잉으로 파형이 망가짐) |

idle 이 3.3V 로 안정적으로 안 잡히고 어중간하게 떠 있으면 그건 신호 문제가 아니라
**해당 핀이 UART 로 muxing 안 된 것**이다.

## 모듈 LED 로 원인 좁히기

TX 모듈 LED 가 **느리게 밝아졌다 어두워지는 걸 반복**하면
"전원은 들어왔는데 유효한 CRSF 를 못 받는 중" 이다.
이 상태에서는 모듈이 송신을 시작하지 않으므로 **RX LED 도 당연히 반응이 없다.**
바인딩 설정 문제가 아니라 CRSF 신호 경로 문제이니, 아래 순서로 좁힌다.

### baud 후보 자동 탐색

Binding phrase 를 양쪽에 맞춰 뒀다면 별도 바인드 절차는 필요 없다 (자동 연결).
그러니 모듈이 CRSF 를 인식하는 baud 를 먼저 찾는다:

```bash
python3 crsf_send.py --baud-sweep
```

후보 baud 를 하나씩 6초간 송신한다. **모듈 LED 를 계속 보고 있다가**
느린 명멸이 멈추고 고정되는 구간의 baud 가 정답이다.

```
400000  ELRS/EdgeTX 계열에서 가장 흔함
420000  CRSF 표준으로 널리 인용되는 값
416666 / 460800 / 115200 / 921600
```

드라이버가 거부하는 baud 는 건너뛰고 계속 진행한다.
찾았으면 그 값으로 다시 확인:

```bash
python3 crsf_send.py --baud <찾은값> --mode hold
```

### 그래도 LED 변화가 없으면

baud 가 아니라 신호가 모듈에 도달하지 않는 것이다:

- **핀 위치** — 모듈 베이의 CRSF 입력 핀에 제대로 꽂혔는지 (모듈 실크/문서 확인)
- **GND 공통** — Jetson GND 와 모듈 GND 가 확실히 묶여 있는지.
  전원을 별도 서플라이에서 받는 경우 특히 놓치기 쉽다
- **핀 8 출력 자체** — 루프백(1단계)을 다시 돌려 UART 가 살아있는지 재확인

### CRSF 는 들어가는데 RX 만 안 붙으면

그때부터 바인딩 영역이다:

- **펌웨어 major 버전** — TX 3.x / RX 2.x 처럼 어긋나면 phrase 가 같아도 안 붙는다
- **주파수 대역** — Ranger Micro 는 2.4GHz. RX 가 900MHz 계열이면 불가능
- **binding phrase** — 대소문자/공백까지 완전히 동일해야 함
...
| `--baud-sweep`     | (off)           | 후보 baud 를 차례로 송신해 탐색        |
| `--sweep-secs`     | `6.0`           | 탐색 시 baud 하나당 송신 시간 [s]      |
