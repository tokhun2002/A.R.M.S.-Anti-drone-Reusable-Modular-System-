...
| 노드                  | 확인 항목                                                    | SITL | 실기체 |
| --------------------- | ------------------------------------------------------------ | ---- | ------ |
| `gz_ros2_bridge`      | 기동 확인, `/arms/image_raw` · `/arms/scan_raw` 발행         | ☐    | —      |
...
Phase 1: Infra
  ...
  [ ] arms_video: Gazebo 카메라 영상 수신 + /arms/image_raw 발행 검증

Phase 2: Perception
  [ ] Roboflow Dataset 기반으로 YOLOv11 small 모델 학습
...
- `SITL_SETUP.md`: 4.1까지 검증 완료
- `RUM.md`: 검증 필요. 나중에 `SITL_SETUP.md` 내용을 합쳐야 함
